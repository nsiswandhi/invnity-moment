...
create or replace function list_published_moments(p_event_id uuid, p_category text, p_cursor text, p_limit integer)
...
begin
  ...
  return (
    with cursor_row as (
      select published_at, id from moments where event_id = p_event_id and id::text = p_cursor and status = 'PUBLISHED' and deleted_at is null
    ),
    page as (
      ...
      where m.event_id = p_event_id and m.status = 'PUBLISHED' and m.deleted_at is null
        ...
        and (p_cursor is null or (m.published_at, m.id) < (select published_at, id from cursor_row))
      group by m.id
    ...
    select jsonb_build_object(
      ...
      'nextCursor', case when (select count(*) from page) > v_limit then (select id::text from returned order by "publishedAt" asc, id asc limit 1) else null end
    )
  );
end; $$;

create or replace function get_published_moment(p_moment_id uuid)
returns jsonb language sql stable as $$
  ...
  from moments m
  where m.id = p_moment_id and m.status = 'PUBLISHED' and m.deleted_at is null
$$;

create or replace function set_public_moment_like(p_moment_id uuid, p_anonymous_user_key_hash text, p_liked boolean)
...
begin
  ...
  if not exists (select 1 from moments where id = p_moment_id and status = 'PUBLISHED' and deleted_at is null) then
