...
export function MomentCard({ moment }: { moment: PublicMoment }) {
  ...
  return <article className="public-moment-card"><Link className="public-moment-link" href={`/album/${moment.id}`}><div className="public-moment-image"><img src={moment.thumbnailUrl} alt={`Momen ${categoryCopy[moment.category].label}`} loading="lazy" /><span className="public-moment-category-badge">{categoryCopy[moment.category].label}</span></div><div className="public-moment-body"><p className="public-moment-caption">{moment.caption}</p><strong>Momen by {moment.participantName} - {moment.participantBatch}</strong></div></Link><div className="public-moment-footer"><time dateTime={moment.publishedAt}>{new Date(moment.publishedAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}</time><button className={`like-button${liked ? ' is-liked' : ''}`} type="button" onClick={() => void toggle()} disabled={busy} aria-pressed={liked} aria-label={`${liked ? 'Batal suka' : 'Suka'} momen, ${likeCount} suka`}>♥ <span>{likeCount}</span></button></div></article>;
}
