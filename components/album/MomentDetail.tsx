...
export function MomentDetail({ momentId }: { momentId: string }) {
  ...
  const download = async () => { if (!moment) return; const response = await fetch(`/api/v1/moments/${moment.id}/download`, { method: 'POST', headers: { 'content-type': 'application/json', 'x-csrf-token': csrfToken() }, body: JSON.stringify({ variant: 'display' }) }); const payload = await response.json() as { data?: { downloadUrl: string }; error?: { message?: string } }; if (!response.ok || !payload.data) { setError(payload.error?.message || 'Download belum dapat dimulai.'); return; } window.location.assign(payload.data.downloadUrl); };
