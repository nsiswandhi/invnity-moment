...
test('participant can register, capture, categorize, and see a saved moment', async ({ page }) => {
  const caption = 'Momen seru bersama Sari.';
  let completedCaption = '';
  ...
  await page.getByRole('button', { name: 'Pilih kategori Reuni' }).click();
  await page.getByLabel(/Cerita untuk momen ini/i).fill(caption);
  await expect(page.getByText('24 / 200', { exact: true })).toBeVisible();
  await expect(page.getByRole('button', { name: /Simpan ke album/i })).toBeEnabled();
  ...
  await page.route('**/api/v1/moments/moment-1/complete', async (route) => { completedCaption = JSON.parse(route.request().postData() || '{}').caption; await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: { moment: { id: 'moment-1', participantId: 'participant-1', eventId: 'event-1', status: 'PUBLISHED', category: 'REUNI', caption, r2OriginalKey: 'original', r2DisplayKey: null, r2ThumbnailKey: null, mimeType: 'image/jpeg', byteSize: 100, width: 640, height: 480, createdAt: '2026-09-10T00:00:00.000Z', publishedAt: '2026-09-10T00:00:00.000Z', deletedAt: null } } }) }); });
  ...
  expect(completedCaption).toBe(caption);
