...
describe('admin security boundary', () => {
  ...
  it('writes an immutable audit record when a moment is hidden', async () => {
    const rpc = vi.fn().mockResolvedValue(undefined);
    const repository = createAdminRepository({ rpc });

    await repository.hideMoment(adminId, momentId, 'Konten tidak sesuai acara.');
  ...
  it('writes a separate immutable audit record when a moment is restored', async () => {
    const rpc = vi.fn().mockResolvedValue(undefined);
    const repository = createAdminRepository({ rpc });

    await repository.unhideMoment(adminId, momentId);
  ...
  it('requires an operator-provided reason before hiding a moment', async () => {
    await expect(hideMoment(adminId, momentId, '   ')).rejects.toMatchObject({
      status: 400,
