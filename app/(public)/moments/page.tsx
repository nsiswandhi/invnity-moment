...
import { EventHeader } from '../../../components/brand/EventHeader';
...
export default function MomentsPage() {
  ...
  const refreshQuota = async () => { const response = await fetch('/api/v1/me'); const payload = await response.json() as MePayload; if (!response.ok || !payload.data?.quota) throw new Error(payload.error?.message || 'Kuota momen belum tersedia.'); setActiveMoments(payload.data.quota?.activeMoments ?? 0); setQuota(payload.data.quota?.maxActiveMoments ?? 10); };
  useEffect(() => {
    let active = true;
    const load = async () => { try { const response = await fetch('/api/v1/me'); const payload = await response.json() as MePayload; if (!response.ok || !payload.data?.participant) throw new Error(payload.error?.message || 'Sesi kamu belum tersedia.'); if (!active) return; const me = payload.data.participant; setParticipant(me); setActiveMoments(payload.data.quota?.activeMoments ?? 0); setQuota(payload.data.quota?.maxActiveMoments ?? 10); const local = readSaved(me.id); const server = await loadOwnedMoments(fetch, null); if (active) setPage(mergeOwnedMoments(server, local)); trackClientEventOnce('my-moments-gallery', 'gallery_view', { source: 'moments' }); } catch (reason) { if (active) setError(reason instanceof Error ? reason.message : 'Sesi kamu belum tersedia.'); } finally { if (active) setLoading(false); } };
    void load(); return () => { active = false; };
  }, [readSaved]);

  const beginCapture = () => { if (activeMoments >= quota) return; setError(''); setScreen('camera'); };
  ...
  const saveUpload = async (retry = false) => {
    ...
    try {
      const resolvedCaption = caption.trim() || DEFAULT_MOMENT_CAPTION;
      ...
      const completeResponse = await fetch(`/api/v1/moments/${reservePayload.data.momentId}/complete`, { method: 'POST', headers: { 'content-type': 'application/json', 'x-csrf-token': csrfToken() }, body: JSON.stringify({ category, caption: resolvedCaption }) }); const completePayload = await completeResponse.json() as { data?: { moment?: MomentRecord }; error?: { message?: string } }; if (!completeResponse.ok || !completePayload.data?.moment) throw new Error(completePayload.error?.message || 'Foto belum tersimpan.');
      const moment = completePayload.data.moment as DisplayMoment; await queue.markSaved(id); await queue.remove(id); const next = [moment, ...page.data]; setPage({ data: next, nextCursor: null }); setActiveMoments((count) => count + 1); writeSaved(participant.id, next); setCaption(''); setUploadState('saved'); setUploadMessage('Momenmu sudah tersimpan!'); trackClientEvent('upload_succeeded', { category }); trackClientEvent('moment_published', { category }); setScreen('success');
  ...
  const deleteMoment = async (moment: MomentRecord) => { if (!participant) return; try { await deleteOwnedMomentRequest(fetch, moment.id, csrfToken()); const next = page.data.filter((item) => item.id !== moment.id); setPage({ data: next, nextCursor: page.nextCursor }); await refreshQuota().catch(() => undefined); writeSaved(participant.id, next); setDeleteTarget(null); setUploadMessage('Satu slot kembali tersedia. Kamu bisa mengambil momen baru.'); trackClientEvent('moment_deleted'); } catch (reason) { setUploadMessage(reason instanceof Error ? reason.message : 'Foto belum dapat dihapus.'); } };
  ...
  if (screen === 'camera') return <main className="site-shell camera-shell"><CameraCapture activeMoments={activeMoments} maxActiveMoments={quota} onCapture={onCapture} onPermissionState={() => undefined} onClose={() => setScreen('home')} /></main>;
  if (screen === 'preview' && photo) return <main className="site-shell"><PhotoPreview blob={photo} onRetake={() => { setCategory(undefined); setCaption(''); setScreen('camera'); }} onSave={() => setScreen('category')} /></main>;
  if (screen === 'category' && photo) return <main className="site-shell form-shell"><Link className="back-link" href="#" onClick={(event) => { event.preventDefault(); setScreen('preview'); }}>← Kembali ke foto</Link><section className="form-card"><p className="eyebrow">Satu langkah lagi</p><h1>Pilih Kategori Foto.</h1><CategoryCards activeCount={activeMoments} maxActiveMoments={quota} selectedCategory={category} onSelect={(selected) => { setCategory(selected); trackClientEvent('category_selected', { category: selected }); }} /><label className="caption-field">Cerita untuk momen ini <span>(opsional)</span><textarea value={caption} maxLength={200} onChange={(event) => setCaption(event.target.value)} placeholder={DEFAULT_MOMENT_CAPTION} aria-describedby="caption-counter" /></label><p id="caption-counter" className="caption-counter" aria-live="polite">{caption.length} / 200</p><button className="primary-button full-button" type="button" disabled={!category} onClick={() => void saveUpload()}>Simpan ke album →</button></section></main>;
  ...
  if (screen === 'success') return <main className="site-shell status-shell"><section className="status-card"><div className="status-orb success-orb" aria-hidden="true">✓</div><p className="eyebrow">Tersimpan di Momen Saya</p><h1>{uploadMessage}</h1><p className="intro-copy">Momenmu ikut menjadi bagian dari cerita reuni.</p><div className="action-row"><button className="secondary-button" type="button" onClick={() => { setPhoto(null); setCategory(undefined); setCaption(''); setScreen('home'); }}>Lihat momen saya</button><button className="primary-button" type="button" onClick={() => { setPhoto(null); setCategory(undefined); setCaption(''); setScreen('camera'); }}>Ambil momen lagi</button></div></section></main>;
  return <main className="site-shell moments-shell"><EventHeader activePage="moments" /><section className="moments-hero"><p className="eyebrow">MOMEN SAYA</p><h1>Hai, {participant.name.split(' ')[0]}!</h1><p className="intro-copy">Simpan cerita reuni yang paling berarti buatmu.</p><div className="quota-badge"><strong>{activeMoments} / {quota}</strong><span>momen tersimpan</span></div><button className="primary-button capture-cta" type="button" onClick={beginCapture} disabled={activeMoments >= quota}>+ Ambil momen</button>{activeMoments >= quota && <p className="muted-copy">Kuota penuh. Hapus satu foto untuk mengembalikan slot.</p>}</section><MyMomentsGrid page={page} onDelete={setDeleteTarget} onCategoryChange={updateCategory} onLoadMore={() => void loadMore()} /><DeleteMomentDialog moment={deleteTarget} onCancel={() => setDeleteTarget(null)} onConfirm={(moment) => void deleteMoment(moment)} />{uploadMessage && <p className="slot-message" role="status">{uploadMessage}</p>}</main>;
}
