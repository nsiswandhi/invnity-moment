...
export default function MomentsPage() {
  ...
  const saveUpload = async (retry = false) => {
    ...
    try {
      ...
      const moment = completePayload.data.moment as DisplayMoment; await queue.markSaved(id); await queue.remove(id); const next = [moment, ...page.data]; setPage({ data: next, nextCursor: null }); writeSaved(participant.id, next); setCaption(''); setUploadState('saved'); setUploadMessage('Momenmu sudah tersimpan!'); trackClientEvent('upload_succeeded', { category }); trackClientEvent('moment_published', { category }); setScreen('success');
