...
export default function MomentsPage() {
  ...
  const deleteMoment = async (moment: MomentRecord) => { if (!participant) return; try { await deleteOwnedMomentRequest(fetch, moment.id, csrfToken()); const next = page.data.filter((item) => item.id !== moment.id); setPage({ data: next, nextCursor: page.nextCursor }); setActiveMoments((count) => Math.max(0, count - 1)); writeSaved(participant.id, next); setDeleteTarget(null); setUploadMessage('Satu slot kembali tersedia. Kamu bisa mengambil momen baru.'); trackClientEvent('moment_deleted'); } catch (reason) { setUploadMessage(reason instanceof Error ? reason.message : 'Foto belum dapat dihapus.'); } };
